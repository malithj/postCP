...
function(formula, data, bp=integer(), family = gaussian(), sigma=1.0, maxFB=FALSE)
{
  ...
  res <- list(model = family, n = n, loglik = loglik, post = post, post.cp = post.cp, cp = cp, le.updated = le.updated, param.before = beta, param.updated = param.updated, response.variable = y)
  class(res) <- append(class(res),"postcp")
  return(res)
}
